...
const std::string FOLDER_PATH = "C:\\Users\\Jiaro\\Desktop\\PinOn\\CrSDK\\pictures\\";
...
int main()
{
    // step1: 自动连接camera，测试成功，返回连接成功信息
    // step2: 监听来自flask的消息，如果是image则拍照，返回找存储地址
    sio::client h;
    ...
    for (;;) {
        ...
        if (SDK::CrSdkControlMode_Remote == camera->get_sdkmode())
        {
            ...
            while (true)
            {
                ...
                h.socket()->on("server_message", [&](sio::event& e) {
                    std::string message = e.get_message()->get_string();
                    std::cout << "Received message from server: " << message << std::endl;
                    // 如果消息是"image"，则拍照并返回地址
                    if (message == "image") {
                        std::cout << "Responding to 'image' message" << std::endl;

                        // set save info
                        count++;
                        std::string seq = intToFiveDigitString(count);
                        std::string now_time = getCurrentDateTime();
                        std::string image_path = FOLDER_PATH + now_time + seq + IMAGE_EXTENSION;
                        std::cout << "image_path: " << image_path << std::endl;

                        cli::text folder_path = convertStringToText(FOLDER_PATH);
                        cli::text time_prefix = convertStringToText(now_time);
                        camera->set_save_info(folder_path, time_prefix, count);

                        // take a photo
                        camera->capture_image();
                        
                        // return image path to flask
                        h.socket()->emit("client_message", image_path);
                    }
                });
